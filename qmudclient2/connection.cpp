...
Connection::Connection(QObject *parent) :
    ...
    m_bFoundJson(false)
{
    connect(&m_tcpSocket, &QTcpSocket::readyRead, this, &Connection::onReadyRead);
    connect(&m_tcpSocket, &QTcpSocket::connected, this, &Connection::connected);
    connect(&m_tcpSocket, &QTcpSocket::disconnected, this, &Connection::disconnected);
}
    ...
    while(true)
    {
        ...
        else
        {
           int len = 0;
            ...
            if (len != 0)
            {
                emit messageReceived(m_buffer.mid(1, len));
